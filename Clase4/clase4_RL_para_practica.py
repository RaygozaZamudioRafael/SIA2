...
#Error de novato salida como entrada cuando se analizan todas las col
x = np.asanyarray(data.drop(columns=['date','price']))
y = np.asanyarray(data[['price']]).ravel() #evitar warnings
...
model = Pipeline([('scaler', StandardScaler()),
                  ('log_reg', LinearRegression())]) #Solver sgd sgt? Se cambio a linear
