...
for ds_cont, ds in enumerate(datasets):
    ...
    #El hiden_layer es la cantidad de neuronas y entre mas estara mas sobreentrenado
    model = MLPClassifier(alpha=0.0, hidden_layer_sizes=(200),max_iter=2000,
                          learning_rate_init=1) #Esto se puede personalizar agregando mas datos como el learnignrte el cual usualmente es maximo 1, mas se undertraining
    #Mas alpha menos entrenado
    ...
    if hasattr(model, 'decision_function'):
        zz = model.decision_functiontion(np.c_[xx.ravel(),yy.ravel()]) #concatenate es reducido a c_
    else:
